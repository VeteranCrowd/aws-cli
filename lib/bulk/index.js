...
export const bulk = new Command()
  ...
  .hook('preSubcommand', async (thisCommand) => {
    ...
    const { command, ignoreErrors, globs, rootPath } = thisCommand.opts();
    const { logger = console } = thisCommand.getdotenvOptions ?? {};

    thisCommand.ignoreErrors = ignoreErrors;
    thisCommand.paths = await globPaths(globs.split(/\s+/), logger, rootPath);
  ...
  .addCommand(
    new Command()
      ...
      .action(async (options, command) => {
        ...
        if (cmd.length)
          await bulkCommand(scripts[cmd] ?? cmd, ignoreErrors, logger, paths);
...
const bulkCommand = async (command, ignoreErrors, logger, paths) => {

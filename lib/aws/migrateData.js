...
export const migrateData = new Command()
  ...
  .option(
    ...
    'max concurrent batchWrite calls during migration',
    '4'
  )
  .option(
    '--max-retries <number>',
    'max retry attempts for batchWrite calls during migration',
    '5'
  )
  .action(
    async (
      ...
      try {
        ...
        writeConcurrency = _.toInteger(writeConcurrency);

        if (writeConcurrency < 1)
          throw new Error('Write concurrency must be a positive integer.');
        ...
        do {
          ...
          if (items.length) {
            await db.putItems(
              ...
              { concurrency: writeConcurrency, maxRetries }
            );

...
export const pullCognito = new Command()
  ...
  .action(async ({ clientName, targetPath, templateExtension }) => {
    ...
    try {
      ...
      _.forOwn(details, async (values, extension) => {
        // Resolve paths.
        const resolvedPaths = targetPath
          ? [resolve(targetPath)]
          : paths.map((p) => resolve(p, `${dotenvToken}.${extension}`));

        await updateDotenv(resolvedPaths, templateExtension, values);
      });
